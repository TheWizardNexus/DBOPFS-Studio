...
// DBOPFS Studio adaptation: importing this class is deliberately side-effect
// free. Studio reports persistence state but only a site's own code may request
// persistent storage.
...
class DBOPFS {
    ...
    */
    ...
    constructor(options={}){
        // DBOPFS Studio can open multiple explicitly selected namespaces without
        // replacing or returning the inspected page's own DBOPFS singleton.
        if(options.singleton!==false&&window.dbopfs){
    ...
    async #init(options={}){
        const scope=await openApplicationDataDirectory({
            ...
            documentObject:Object.prototype.hasOwnProperty.call(options,'documentObject')
                ?options.documentObject
                :globalThis.document,
            arcane:Object.prototype.hasOwnProperty.call(options,'arcane')
                ?options.arcane
                :globalThis.Arcane,
        ...
        if(options.initializeDefaultTables!==false){
            await this.#createDefaultTables();
        }

        this.ready=true;

        if(options.dispatchReadyEvent!==false){
...
// DBOPFS Studio adaptation: never create a database merely by importing the
// module. The page agent constructs an instance only after an app is selected.
